...
FocusScope {
    ...
    implicitWidth: layout === "row" ? 
        (buttonSize * actions.length + spacing * (actions.length - 1)) : 
        (buttonSize * Math.min(columns, actions.length) + spacing * (Math.min(columns, actions.length) - 1))
    implicitHeight: layout === "row" ? 
        buttonSize : 
        (Math.ceil(actions.length / columns) * buttonSize + spacing * (Math.ceil(actions.length / columns) - 1))
        ...
        Grid {
            ...
            Repeater {
                ...
                delegate: Button {
                    ...
                    background: BgRect {
                        color: actionButton.pressed ? Colors.adapter.primary : 
                               (actionButton.hovered || actionButton.activeFocus) ? Colors.adapter.surfaceContainerHighest : 
                               Colors.adapter.surfaceContainer
